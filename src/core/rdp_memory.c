...
bool sr_memory_read_be16(const sr_memory *mem, uint32_t addr, uint16_t *value)
{
    ...
    *value = ((uint16_t)mem->rdram[addr] << 8) |
             ((uint16_t)mem->rdram[addr + 1u]);
    return true;
}
...
bool sr_memory_read_be32(const sr_memory *mem, uint32_t addr, uint32_t *value)
{
    ...
    *value = ((uint32_t)mem->rdram[addr] << 24) |
             ((uint32_t)mem->rdram[addr + 1u] << 16) |
             ((uint32_t)mem->rdram[addr + 2u] << 8) |
             ((uint32_t)mem->rdram[addr + 3u]);
    return true;
}
...
bool sr_memory_write_be16(sr_memory *mem, uint32_t addr, uint16_t value)
{
    ...
    mem->rdram[addr] = (uint8_t)(value >> 8);
    mem->rdram[addr + 1u] = (uint8_t)value;
    return true;
}
...
bool sr_memory_write_be32(sr_memory *mem, uint32_t addr, uint32_t value)
{
    ...
    mem->rdram[addr] = (uint8_t)(value >> 24);
    mem->rdram[addr + 1u] = (uint8_t)(value >> 16);
    mem->rdram[addr + 2u] = (uint8_t)(value >> 8);
    mem->rdram[addr + 3u] = (uint8_t)value;
    return true;
}
